...
@interface TiApiAttModule : TiModule <NSURLConnectionDelegate,NSURLConnectionDataDelegate,MessageHTTPHandling>
{
    ...
    KrollCallback *successWapPushCallback;
    KrollCallback *errorWapPushCallback;
@private
    NSMutableData*      mms_response;
    bool                is_debug;
...
/** 
 * Name:sendWapPush
 * Description:It allows to send WAP Push requests to supported devices on the AT&T Mobility network.
 * Argument (args):Array contains WAP Push parameter with success and failure callback.
 * Return  :void
 */
