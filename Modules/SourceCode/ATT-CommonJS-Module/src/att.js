var tiRequire=require;(function(){
/**
...
define('att/util',['require','exports','module'],function(require, exports, module) {
	...
	exports.apply = function(scope, method, args) {
		return apply.call(method, scope, args);
	};
...
define('att/ajax',['require','exports','module','att/util','att/constants'],function(require, exports, module) {
	...
	,	valuesToExpose = ['readyState', 'responseText', 'responseData', 'responseXML', 'responseType']
	...
	function AttXHR(params, successCB, failCB) {
		...
		function processResponseData() {
			return (attXHR.getResponseHeader('Content-Type').indexOf(constants.header.contentType.JSON) >= 0) ?
		...
		if(params.query) {
			...
			query = (queryType === 'object') ? util.qs.stringify(query) :
		...
		//Set up the timeout
		xhr.timeout = (params.timeout !== undefined) ? params.timeout : ajax.settings.timeout;
		if(false && xhr.timeout > 0) {
			//XMLHttpRequest doesn't have timeout built in on all browsers
			timeoutHandle = setTimeout(function() {
				timeoutHandle = null;
				xhr.abort();
				onReply(failCB, generateErrorObj('AttXHRTimeoutError', 'Request aborted'));
			}, xhr.timeout);
		}
		
		var body = params.body;
		if(headers['Content-Type'] === constants.header.contentType.JSON && typeof params.body === 'object') {
 ...
 *  
 *  @method addObject {function(name, newObject)} - Attaches a new child ATTObject/ATTMethod to the current ATTObject.
 *  @method addMethod {function(name, method, params)} - Attaches a new child ATTMethod to the current ATTObject.
 *  @method getRoot {function(): ATTObject} - returns the root of the ATTObject tree
 *  @method forEachToRoot {function(func, scope)} - Simulates a Array.forEach function running from child to the root object
 *
 *  @method getHeaderConfig {function(): Object} - builds and returns config objects for headers for the given ATTObject
 *  @method getQueryStringConfig {function(): Object} - builds and returns config objects for query strings for the given ATTObject
 *	@method getUrlParamsConfig {function(): Object} - builds and returns config objects for URL parameters for the given ATTObject
 *	
 ...
 *  			and total inferUrlParamsConfig which is generated from appendUrl's of the given ATTObject
 *  
 ...
 *  
 */
/** Function ATTMethod extends ATTObject
...
define('att/ATTObject',['require','exports','module','att/util','att/constants'],function(require, exports, module) {
	var util = require('att/util')
	...
	function ATTObject(props) {
		copyObjTo(this, props);
	}
	copyObjTo(ATTObject.prototype, {
		addObject: function(name, newObject) {
			if(!(newObject instanceof ATTObject) && typeof newObject !== 'function') {
				throw new Error('Only ATTObject or ATTMethod can be added with this method');
			}
			...
			objPath.forEach(function(pathObjName) {
				if(!target[pathObjName]) target[pathObjName] = new ATTObject();
		...
		addMethod: function(methodName, newMethod, props) {
			if(typeof newMethod === 'object' && typeof props === 'undefined') {
			...
			copyObjTo(ATTMethod, props, ATTObject.prototype, ATTObject.defaultMethodParams);
			ATTMethod.executor = newMethod;
			ATTMethod.methodName = methodName && methodName.match(/\.?([^.]*)$/)[1];
			
			this.addObject(methodName, ATTMethod); //This method adds the parent reference
		},
	...
	['headerConfig', 'queryStringConfig', 'urlParamsConfig'].forEach(function(configType) {
		...
		ATTObject.prototype[getMethodName] = function() {
	...
	ATTObject.prototype.getUrl.paramsRegex = /(\/):([^\/?]*)(\?$)?/g;
	
	ATTObject.defaultMethodParams = {};
	
	module.exports = ATTObject;
});


define('att/main',['require','exports','module','att/ajax','att/util','att/constants','att/ATTObject'],function(require, exports, module) {
	...
	/**
	 * @class ATTObject (at the root only)
	...
	var ATTObject = require('att/ATTObject');
	...
	*/
	...
	function baseExecutor(attMethod, params, successCB, failCB) {
		...
		var buildRequest = function(accessToken) {
	...
	var ATT = module.exports = new ATTObject({
	...
	ATTObject.defaultMethodParams.tokenType = ATT.TokenType.ACCESS;
	
	var appKey, secret, accessScopeStr, authScopeStr, oAuthCode
	,	possibleAuthScope = util.arrayToHash(['IMMN','MIM','TL']);
	
	
	ATT.addObject('OAuth', new ATTObject({
		appendUrl: '/oauth',
		methods: {
			'getAccessToken': {
			...
			'obtainEndUserAuthorization': { //TODO This may need to be removed from here
				method: 'GET',
				appendUrl: '/authorize',
				tokenType: ATT.TokenType.NONE,
				queryStringConfig: {
	...
	copyObjTo(TokenObject.prototype, {
		fetch: function(data, success, fail) {
			...
			if(this.isExpired()) {
				...
				accessParams.refresh_token = this.refresh;
			} else if(this.token) {
				//Return the token if we have one cached
				success.call(this, this.token);
			...
			var mySuccess = function(resp, ajax) {
				...
				var formattedResp = self.formatResponse(resp.data);
				self.saveToken(formattedResp);
				self.cacheToken(formattedResp);
				
				if(success) success.call(self, self.token);
			};
		...
		formatResponse: function(tokenResp) {
		...
		clearCachedToken: function() {
			...
			['token', 'refresh', 'expiration'].forEach(function(key) {
				delete this[key];
			}, this);
		},
		cacheToken: function(tokenData) {
			this.token = tokenData.t;
			this.refresh = tokenData.r;
			this.expiration = tokenData.e;
		},
		isExpired: function() {
			return this.refresh && this.expiration !== 0 && this.expiration < Date.now();
		},
		//Delete the persisted token
		deleteToken: function() {}, 		 //Place Holder. Create this for the given platform
		//Persist the token
		saveToken: function(tokenData) {},   //Place Holder. Create this for the given platform
		//load the persisted token into memory
		loadToken: function() {} 			 //Place Holder. Create this for the given platform
	});
	
	ATT.accessToken = new TokenObject({
		formatParameters: function(accessParams) {
			if(!accessScopeStr) {
	...
	ATT.userAuthToken = new TokenObject({
	...
	//Parsing of the passed in scope to prevent confusion in the separate scope
	function setScope(newScopeArr) {
		var accessScope = {}
		,	authScope = {};
		
		newScopeArr.forEach(function(scopeVal) {
		...
		accessScopeStr = Object.keys(accessScope).sort().join(',');
		authScopeStr = Object.keys(authScope).sort().join(',');
	}
	
	function isSameScope(newScopeArr) {
		var currentScope = util.arrayToHash(
				accessScopeStr ? accessScopeStr.split(',') : [], 
				authScopeStr ? authScopeStr.split(',') : []
		);
		
		if(newScopeArr.length !== Object.keys(currentScope).length) return false;
		
		return newScopeArr.every(function(scopeVal) {
			return currentScope[scopeVal];
		});
	}
	
	ATT.setKeys = function(newAppKey, newSecret, newScope) {
		newScopeArr = newScope ? newScope.split(',') : [];
		if(appKey === newAppKey && secret === newSecret && isSameScope(newScopeArr)) {
			return; //Nothing changed so do nothing
		}
		
		ATT.accessToken.clearCachedToken();
		ATT.userAuthToken.clearCachedToken();
		
		appKey = newAppKey;
		secret = newSecret;
		setScope(newScopeArr);
	};
	...
	if(true) {
		ATT.addObject('SMS', new ATTObject({
	...
	if(true) {
		ATT.addObject('MMS', new ATTObject({
	...
	if(true) {
		(function() {
			...
			ATT.addObject('Payment', new ATTObject({
	...
	if(true) {
		ATT.addObject('IMMN', new ATTObject({
	...
	if(true) {
		ATT.addObject('CMS', new ATTObject({
	...
	if(false) {
		ATT.addObject('Ads', new ATTObject({
	...
	if(false) {
		ATT.addObject('WAPPush', new ATTObject({
	...
	if(true) {
		ATT.addObject('Location', new ATTObject({
	...
	if(true || false || true) {
		ATT.addObject('Speech', new ATTObject({
			appendUrl: '/speech/v3/'
		}));
	}
	...
	if(false) {
		...
		/*
		ATT.addObject('TTS', new ATTObject({
	...
	ATT.addObject('Notary', new ATTObject({
		...
		methods: {
			'signedPayload': {
				...
				executor: function(formattedParams, params) {
					var token = this.getRoot().accessToken.token;
...
require(['att/main', 'att/util', 'att/ajax', 'att/constants', 'att/ajax.mime'], function(ATT, util, ajax, constants, mime) {
	...
	if(false) {
		...
		var ttsFunc = ATT.TTS.textToSpeech;
...
define('wrappers/wrapper-1.0',['att/main', 'att/constants', 'att/util'], function(attBase, constants, util) {
	...
	if(true) {
		(function() {
			...
			function getNotificationFunction(method) {
				return function(params, success, fail) {
					attBase.Payment[method]({
						headers: getCommonHeaders(params),
						urlParams: {
	...
	var excludeMethodList = ['speechToTextCustom'];
	
	util.forEach(ATT, function(methods, scope) {
		if(typeof methods === 'object') util.forEach(methods, function(method, methodName) {
			if(excludeMethodList.indexOf(methodName) >= 0) return;
			methods[methodName] = formatMethodInput(method, [scope, methodName]);
		});
	});
	
	return ATT;
});var r=require;r("extensions/titanium");exports.ATT=r("wrappers/wrapper-1.0");})();