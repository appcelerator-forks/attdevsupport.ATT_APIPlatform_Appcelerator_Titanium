...
- (void) releaseCallback
{
    ...
    RELEASE_TO_NIL(errorWapPushCallback);
    RELEASE_TO_NIL(successWapPushCallback);
}
...
#pragma mark-
#pragma mark WAP-PUSH APIs

-(void)sendWapPush:(id)args
{
    ENSURE_UI_THREAD_1_ARG(args);
    
    if (successWapPushCallback) {
        RELEASE_TO_NIL(successWapPushCallback);
    }
    if (errorWapPushCallback) {
        RELEASE_TO_NIL(errorWapPushCallback);
    }
    successWapPushCallback = [[args objectAtIndex:1] retain];
    errorWapPushCallback = [[args objectAtIndex:2] retain];
    ...
    if(is_debug)
    {
        NSLog(@"[INFO]Wap Push All Header Set::%@",[request allHTTPHeaderFields]);
        NSLog(@"[INFO]Wap Push Body ::%@",[[NSString alloc] initWithData:body encoding:NSUTF8StringEncoding]);
    }
...
- (void) messageRequest:(MessageHTTPRequest*)request didCompleteRequestWithData:(NSData*)data
{
    ...
    if (successWapPushCallback) {
            [successWapPushCallback call:[NSArray arrayWithObject:responseObject] thisObject:nil];
    }
...
- (void) sendErrorMessage:(const NSString*)message
{
    ...
    if (errorWapPushCallback) {
        [errorWapPushCallback call:[NSArray arrayWithObject:errorString] thisObject:nil];
    }
    [self releaseCallback];
}
